...
export default function RewritesPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const lines = [
        `# DNS Rewrites Export — ${date}`,
        `# Total: ${all.length} entries`,
        `# Generated by rust-dns`,
        '',
        ...all.map(r => `${r.answer}\t${r.domain}`),
        '',
      ];
      ...
      a.href = url;
      a.download = `rewrites-export-${date}.hosts`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast.success(t('rewrites.exportSuccess', { count: all.length }));
    } catch {
      toast.error(t('rewrites.exportFailed'));
    } finally {
      setExporting(false);
    }
  };
